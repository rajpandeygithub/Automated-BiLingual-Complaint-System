...
_pipeline_artifacts_dir = (
    f"gs://{GCS_artifacts_bucket_name}/pipeline_artifacts/{pipeline_name}"
)
...
def get_data_component(
    ...
    try:
        ...
        train, val = train_test_split(
            data_features, test_size=testset_size, random_state=42
        )
...
def test_xgboost_model(
    ...
    def send_slack_message(
        ...
        if recall is not None:
            message["attachments"][0]["fields"].append(
                {"title": "Validation Recall", "value": f"{recall:.4f}", "short": True}
            )
...
# register
@component(packages_to_install=["google-cloud-aiplatform", "google-auth"])
...
def model_deployment(
    ...
    # Function to send custom Slack message with Kubeflow component details
    def send_slack_message(
        component_name, execution_date, execution_time, duration, endpoint_name
    ):
        message = {
            "attachments": [
                {
    ...
    try:
        ...
        if deployed_model_resource is not None and hasattr(
            deployed_model_resource, "id"
        ):
            # Retrieve the current traffic allocation and set traffic of old versions to 0%
            traffic_split = {
                deployed_model_resource.id: 100
            }  # New model gets 100% traffic
            for deployed_model_id in endpoint_obj.traffic_split.keys():
                if deployed_model_id != deployed_model_resource.id:
                    traffic_split[deployed_model_id] = (
                        0  # Set old versions to 0% traffic
                    )
    ...
    try:
        ...
        rows_to_insert = [
            {"last_training_timestamp": current_timestamp, "record_count": record_count}
        ]
...
def bias_detection(
    ...
    def send_slack_message(
        ...
        if alerts:
            ...
            for alert in alerts:
                message["attachments"][0]["fields"].append(
                    {"title": "Bias Alert", "value": alert, "short": False}
                )
        ...
        try:
            sensitive_features = (
                "product"  # We are now only checking for the 'product' feature
            )
            ...
            metric_frame = MetricFrame(
                ...
                sensitive_features=data[
                    sensitive_features
                ],  # We are checking for the 'product' feature
            )
            ...
            slice_results.columns = [
                "Product",
                "Accuracy",
            ]  # Ensuring column names are correct for Slack
            ...
            def check_for_bias(slice_metrics, threshold):
                ...
                for slice_name, accuracy in slice_metrics.items():
                    if accuracy < threshold:
                        alert_flag = True
                        alerts.append(
                            f"Bias Alert: Accuracy for slice '{slice_name}' is below threshold: {accuracy:.2f}. We are removing these classes for mitigation."
                        )
...
parser.add_argument(
    "--slack_url", required=True, help="Slack Webhook URL"
