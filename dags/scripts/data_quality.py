...
def validate_and_transform_dates(df):
    ...
    df = df.with_columns(
        [pl.col(field).str.strptime(pl.Date, format="%Y-%m-%d").cast(pl.Date) for field in date_fields]
    ...
    if future_received.height > 0:
        logger.info(f"Warning: {future_received.height} records have 'date_received' in the future.")
    ...
    if invalid_resolution.height > 0:
        logger.info(f"Warning: {invalid_resolution.height} records have 'date_resolved' earlier than 'date_received'.")
...
def validate_numeric_fields(df):
    ...
    df = df.with_columns(
        pl.col('complaint_id').cast(pl.Int64).alias('complaint_id')
    )
    logger.info("First step done")

    invalid_complaint_ids = df.filter(pl.col('complaint_id') <= 0)
    if invalid_complaint_ids.height > 0:
        logger.info(f"Warning: {invalid_complaint_ids.height} records have non-positive 'complaint_id'.")
    ...
    if negative_resolution_times.height > 0:
        logger.info(f"Warning: {negative_resolution_times.height} records have negative 'time_resolved_in_days'.")
        df = df.with_columns(
            pl.when(pl.col('time_resolved_in_days') < 0).then(0).otherwise(pl.col('time_resolved_in_days')).alias('time_resolved_in_days')
        )
...
def validate_text_fields(df):
    ...
    if long_complaints.height > 0:
        logger.info(f"Warning: {long_complaints.height} complaints exceed 5000 characters.")
        df = df.with_columns(
            pl.col('complaint').str.slice(0, 5000).alias('complaint')
        )
...
def validate_issues_and_sub_issues(df, required_issues):
    ...
    missing_sub_issues = df.filter(
        ...
        (pl.col('sub_issue').is_null() | (pl.col('sub_issue').str.strip_chars("") == ""))
    )
    
    if missing_sub_issues.height > 0:
        logger.info(f"Warning: {missing_sub_issues.height} records have missing 'sub_issue' for required 'issue':")
...
def validate_interdependent_constraints(df):
    ...
    if invalid_responses.height > 0:
        logger.info(f"Warning: {invalid_responses.height} records violate the constraint:")
        logger.info(invalid_responses[['consumer_disputed', 'company_response_consumer']])
...
def validate_date_constraints(df):
    ...
    if invalid_dates.height > 0:
        logger.info(f"Warning: {invalid_dates.height} records have 'date_sent_to_company' earlier than 'date_received':")

    return df

def validate_zipcode(df):
    df = df.with_columns(
        pl.col('zipcode').cast(pl.Utf8).alias('zipcode')
    )
    ...
    if invalid_zipcodes.height > 0:
        logger.info(f"Warning: {invalid_zipcodes.height} records have invalid ZIP codes:")
...
def validate_enum_values(df, feature_name, valid_values):
    if feature_name in df.columns:
        ...
        df = df.with_columns(
            pl.when(invalid_mask).then(None).otherwise(pl.col(feature_name)).alias(feature_name)
        )
...
def validate_data_quality(dataset: str)-> str:
    ...
    raw_dataset = raw_dataset.with_columns(
        pl.when(pl.col('state').is_null()).then(pl.lit(None)).otherwise(pl.col('state')).alias('state'),
        pl.when(pl.col('tags').is_null()).then(pl.lit('None')).otherwise(pl.col('tags')).alias('tags')
    )
    ...
    raw_dataset = validate_enum_values(raw_dataset, 'timely_response', valid_timely_response)
    # Validate the 'consumer_disputed' field
    raw_dataset = validate_enum_values(raw_dataset, 'consumer_disputed', valid_consumer_disputed)
