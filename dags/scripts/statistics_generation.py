...
def identify_complaint_outliers(dataframe, max_length=6000):
    ...
    dataframe = dataframe.with_columns(pl.col('complaint').str.len_chars().alias('complaint_length'))
    ...
    logging.info(f"Percentage of outliers: {outliers.shape[0] / dataframe.shape[0] * 100:.2f}%")

    # Log some information about the outliers
    if outliers.shape[0] > 0:
        logging.info("\nSample of outliers:")
        logging.info(outliers.select(['complaint', 'complaint_length']).head().to_pandas().to_string())
...
def consistency_checks(df):
    ...
    if inconsistent_dates.shape[0] > 0:
        issues.append(f"Found {inconsistent_dates.shape[0]} complaints where resolution date is before received date")
...
def generate_summary_report(df, schema, anomalies, consistency_issues):
    ...
    report.append(f"- Date range: {df['date_received'].min().strftime('%Y-%m-%d')} to {df['date_received'].max().strftime('%Y-%m-%d')}")
    ...
    report.append("\n## Data Distribution")
    report.append("\n### Top 5 Products:")
    for product, percentage in df['product'].value_counts(normalize=True).head().iter_rows():
        report.append(f"- {product}: {percentage:.2%}")

    report.append("\n### Top 5 Issues:")
    for issue, percentage in df['issue'].value_counts(normalize=True).head().iter_rows():
...
def schema_and_statistics_generation(dataset: str)-> str:
    ...
    for cat_feature in ['product', 'department', 'issue']:
        ...
        minority_classes = distribution.filter(pl.col('proportion') < imbalance_threshold)
        if minority_classes.shape[0] > 0:
            logging.warning(f"Found {minority_classes.shape[0]} minority {cat_feature}s with less than {imbalance_threshold:.1%} representation:")
    ...
    yearly_changes = yearly_stats.with_columns([
        (pl.col('complaint_count') / pl.col('complaint_count').shift(1) - 1).alias('complaint_change'),
        (pl.col('product_count') / pl.col('product_count').shift(1) - 1).alias('product_change'),
        (pl.col('issue_count') / pl.col('issue_count').shift(1) - 1).alias('issue_change')
    ])
    ...
    if duplicates.shape[0] > 0:
        ...
        duplicate_rows = df.join(duplicates.select("complaint"), on="complaint", how="inner")
