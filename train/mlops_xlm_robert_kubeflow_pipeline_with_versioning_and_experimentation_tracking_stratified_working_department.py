# -*- coding: utf-8 -*-
"""MLOps_XLM_RoBERT_KubeFlow_Pipeline_with_Versioning_and_Experimentation_Tracking_Stratified_Working_Department.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1BJq9B0j85vpfpm1HODgE4CDf_DxHaBKe
"""
...
def get_data_component(
  ...
  try:
    ...
    QUERY = f'''select complaint_english, complaint_hindi, department from (select *, row_number() over(partition by department) as rank_order from `bilingualcomplaint-system.MLOps`.get_dataset_by_complaint_year_interval(2018, 2023) where 1=1 and department in ( select department from `bilingualcomplaint-system.MLOps`.get_dataset_by_complaint_year_interval(2018, 2023) group by department having count(department) >= 468 ) ) where rank_order <= 468'''
    ...
    products = data['department'].tolist()
    ...
    print("Melted DF Size: {}".format(data_features.shape))
...
def train_mbert_model(
    ...
    label_map: Dict[str, int],
    model_save_name: str = 'saved_xlmroberta_dep_model',
  ...
  def build_bert_model(vocab_size, embedding_dim=128, lstm_units=64, num_classes=2):
      model = TFXLMRobertaForSequenceClassification.from_pretrained(huggingface_model_name, num_labels=num_classes)
      return model
...
def test_mbert_model(
    ...
    test_data_name: str,
    label_map: Dict[str, int],
    model_name: str = 'saved_xlmroberta_dep_model',
    batch_size: int = 8,
    max_sequence_length: int = 128,
    huggingface_model_name: str = 'xlm-roberta-base'
):
    ...
    try:
      ...
      metrics = {'precision_department': precision, 'recall_department': recall, 'f1_department': f1,
                 'precision_per_label_department': json.dumps(precision_per_label.tolist()), 'recall_per_label_department': json.dumps(recall_per_label.tolist()), 'f1_per_label_department': json.dumps(f1_per_label.tolist())}
...
label_2_idx_map = {label: idx for idx, label in enumerate(department_labels)}
idx_2_label_map = {idx: label for label, idx in label_2_idx_map.items()}

bert_model_params = {}

@pipeline(
    name="department_stratify_pipeline_with_bias_and_experimentation_tracking_xlm_robert",
...
def model_data_pipeline(start_year: int = 2018, end_year: int = 2023):
    ...
    register_model_task = model_registration(
        ...
        project_id=PROJECT_ID,
        location=LOCATION,
        model_display_name="tf_xlm_robert_model_department",
    )
    ...
    deploy_model_task = model_deployment(
        ...
        project_id=PROJECT_ID,
        location=LOCATION,
        endpoint_display_name="tf_xlm_robert_endpoint_department",
        deployed_model_display_name="tf_xlm_robert_deployed_department",
    )
...
compiler.Compiler().compile(
    pipeline_func=model_data_pipeline, package_path="model_deployment_pipeline_department_job_xlm_robert.json"
)

TIMESTAMP = datetime.now().strftime("%Y%m%d%H%M%S")

job = aiplatform.PipelineJob(
    display_name="model_deployment_pipeline_department-xlm-robert",
    template_path="model_deployment_pipeline_department_job_xlm_robert.json",
    job_id="model-deployment-department-pipelinexlm-robert-{0}".format(TIMESTAMP),
    enable_caching=True
)
